      ...
      .map((line, index) => {
        ...
        return {
          ...
          input: {
            ...
            housingStatus: columns[6] || 'no_housing_costs',
            tenantType: columns[7] || 'private',
            rent: parseFloat(columns[8]) || 0,
            serviceCharges: parseFloat(columns[9]) || 0,
            bedrooms: parseInt(columns[10]) || 1,
            area: columns[11] || 'default',
            nonDependants: parseInt(columns[12]) || 0,
            employmentType: columns[13] || 'not_working',
            monthlyEarnings: parseFloat(columns[14]) || 0,
            childcareCosts: parseFloat(columns[15]) || 0,
            savings: parseFloat(columns[16]) || 0,
            otherBenefits: parseFloat(columns[17]) || 0,
            otherBenefitsPeriod: columns[18] || 'monthly',
            // Add more fields as needed
          },
          expectedOutput: {
            standardAllowance: parseFloat(columns[19]) || 0,
            housingElement: parseFloat(columns[20]) || 0,
            childElement: parseFloat(columns[21]) || 0,
            childcareElement: parseFloat(columns[22]) || 0,
            carerElement: parseFloat(columns[23]) || 0,
            totalElements: parseFloat(columns[24]) || 0,
            earningsReduction: parseFloat(columns[25]) || 0,
            capitalDeduction: parseFloat(columns[26]) || 0,
            benefitDeduction: parseFloat(columns[27]) || 0,
            finalAmount: parseFloat(columns[28]) || 0,
          }
        };
      });
  };
