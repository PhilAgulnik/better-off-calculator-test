      ...
      .map((line, index) => {
        ...
        return {
          ...
          input: {
            ...
            tenantType: columns[8] || 'private',
            rent: parseFloat(columns[9]) || 0,
            serviceCharges: parseFloat(columns[10]) || 0,
            bedrooms: parseInt(columns[11]) || 1,
            area: columns[12] || 'default',
            nonDependants: parseInt(columns[13]) || 0,
            employmentType: columns[14] || 'not_working',
            monthlyEarnings: parseFloat(columns[15]) || 0,
            childcareCosts: parseFloat(columns[16]) || 0,
            savings: parseFloat(columns[17]) || 0,
            otherBenefits: parseFloat(columns[18]) || 0,
            otherBenefitsPeriod: columns[19] || 'monthly',
            // Add more fields as needed
          },
          expectedOutput: {
            standardAllowance: parseFloat(columns[20]) || 0,
            housingElement: parseFloat(columns[21]) || 0,
            childElement: parseFloat(columns[22]) || 0,
            childcareElement: parseFloat(columns[23]) || 0,
            carerElement: parseFloat(columns[24]) || 0,
            totalElements: parseFloat(columns[25]) || 0,
            earningsReduction: parseFloat(columns[26]) || 0,
            capitalDeduction: parseFloat(columns[27]) || 0,
            benefitDeduction: parseFloat(columns[28]) || 0,
            finalAmount: parseFloat(columns[29]) || 0,
          }
        };
      });
  };
