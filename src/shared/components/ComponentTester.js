...
import { UniversalCreditCalculator } from '../../features/uc-calculator/utils/calculator';
      ...
      .map((line, index) => {
        ...
        return {
          ...
          input: {
            ...
            pipDailyLivingRate: columns[21] || 'none',
            pipMobilityRate: columns[22] || 'none',
            dlaCareRate: columns[23] || 'none',
            dlaMobilityRate: columns[24] || 'none',
            aaRate: columns[25] || 'none',
            hasLCWRA: columns[26] || 'no',
            // Partner disability fields
            partnerEmploymentType: columns[27] || 'not_working',
            partnerMonthlyEarnings: parseFloat(columns[28]) || 0,
            partnerIsDisabled: columns[29] || 'no',
            partnerClaimsDisabilityBenefits: columns[30] || 'no',
            partnerDisabilityBenefitType: columns[31] || '',
            partnerPipDailyLivingRate: columns[32] || 'none',
            partnerPipMobilityRate: columns[33] || 'none',
            partnerDlaCareRate: columns[34] || 'none',
            partnerDlaMobilityRate: columns[35] || 'none',
            partnerAaRate: columns[36] || 'none',
            partnerHasLCWRA: columns[37] || 'no',
            savings: parseFloat(columns[38]) || 0,
            otherBenefits: parseFloat(columns[39]) || 0,
            otherBenefitsPeriod: columns[40] || 'monthly',
