...
export class ChildBenefitCalculator {
  constructor() {
    this.rates = {
      ...
      '2024_25': {
        eldestChild: 25.60, // per week - Official GOV.UK rate
        additionalChildren: 16.95 // per week - Official GOV.UK rate
      },
      '2023_24': {
        eldestChild: 24.00, // per week - Official GOV.UK rate
        additionalChildren: 15.90 // per week - Official GOV.UK rate
      }
    };
  }
  ...
  getHighIncomeChargeMessage(formData) {
    ...
    if (yearlyEarnings >= 80000) {
      return {
        type: 'full_charge',
        message: 'You will be subject to the Child Benefit high income tax charge. This charge will be the same value as your Child Benefit. You may choose to stop receiving Child Benefit but if you claim it and repay the relevant tax you will also get credits towards your state pension. To find out more about your options see {Child Benefit charge}.',
  ...
  calculateYearlyEarnings(formData) {
    ...
    if (formData.employmentType === 'self-employed') {
      const businessIncome = (parseFloat(formData.businessIncomeBank) || 0) + (parseFloat(formData.businessIncomeCash) || 0);
      ...
      const monthlyBusinessExpenses = this.convertToMonthly(businessExpenses, businessExpensesPeriod);
