...
export class BenefitCalculator {
  ...
  getCalculationBreakdown(formData) {
    ...
    if (results.universalCredit.calculation) {
      ...
      if (calc.standardAllowance > 0) ucBreakdown.push({ label: 'Standard Allowance', amount: calc.standardAllowance });
      if (calc.housingElement > 0) ucBreakdown.push({ label: 'Housing Element', amount: calc.housingElement });
      if (calc.childElement > 0) ucBreakdown.push({ label: 'Child Element', amount: calc.childElement });
